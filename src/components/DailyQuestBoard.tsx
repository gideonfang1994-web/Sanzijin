...
import { CheckCircle2, Swords, BookOpen, Gamepad2, ChevronRight, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
...
const DailyQuestBoard: React.FC<Props> = ({ quests = [], onQuestClick }) => {
  ...
  return (
    <div className="bg-gradient-to-b from-white via-[#f6fcf8] to-[#edfcf4] backdrop-blur-md rounded-[24px] p-3 border-2 border-emerald-250 mb-3 relative overflow-hidden shadow-md">
      ...
      <div className="absolute -bottom-16 -left-16 w-36 h-36 bg-yellow-150/50 rounded-full blur-2xl pointer-events-none" />
        ...
        <div 
          ...
          className="flex items-center justify-between gap-1 relative z-10 w-full cursor-pointer select-none"
        >
          <div className="flex items-center space-x-2 min-w-0">
            ...
            <div className="min-w-0 text-left">
              <h3 className="text-sm sm:text-base font-sans font-black text-emerald-950 tracking-wide flex items-center gap-1 leading-tight whitespace-nowrap">
                <span>今日探险任务</span>
                <Sparkles size={14} className="text-amber-500 animate-pulse shrink-0 hidden sm:inline" />
              </h3>
              <span className="text-[9px] sm:text-[10px] text-emerald-700/80 font-extrabold tracking-wider block mt-0.5 uppercase whitespace-nowrap">
                {isExpanded ? '点击收起任务 ▲' : '点击下拉任务 ▼'}
              </span>
            </div>
          </div>
          ...
          <div className="shrink-0 flex items-center gap-1.55">
            ...
            <div className="w-5.5 h-5.5 sm:w-6 sm:h-6 rounded-full bg-emerald-105 border border-emerald-200 flex items-center justify-center text-emerald-800 shadow-3xs shrink-0">
              {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} className="animate-bounce" />}
            </div>
          </div>
        </div>
      ...
      {isExpanded && (
        ...
        <div className="space-y-2 relative z-10 mt-3 pt-3 border-t-2 border-dashed border-emerald-200/50">
