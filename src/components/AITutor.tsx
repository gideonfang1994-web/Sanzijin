...
const AITutor: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-[44px] overflow-hidden shadow-puffy border-[1.5px] border-slate-100">
      {/* Header Container */}
      <div className="bg-gradient-to-r from-blue-600 to-indigo-500 p-6 flex items-center text-white shadow-lg z-10 relative">
        ...
        <div>
          ...
          <div className="flex items-center mt-1">
            ...
            <p className="text-xs font-bold text-blue-100 uppercase tracking-widest">Mastering Rhythms</p>
          </div>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto space-y-6 bg-[#FAFBFE] scroll-smooth">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-4 duration-500`}>
            {m.role === 'ai' && (
              <div className="w-9 h-9 rounded-xl bg-blue-100 flex items-center justify-center mr-3 mt-auto shadow-sm text-lg">🐯</div>
            )}
        ...
        <div className="flex items-center space-x-3">
          <div className="flex-1 relative group">
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-[24px] px-6 py-4.5 focus:outline-none focus:ring-4 focus:ring-blue-100 focus:bg-white focus:border-blue-400 transition-all font-bold pr-14 placeholder:text-slate-300"
            />
            <Smile className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 w-6 h-6 hover:text-blue-500 transition-colors cursor-pointer" />
          </div>
          <button 
            onClick={handleSend}
